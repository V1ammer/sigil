...
impl ClientIdentity {
    ...
    /// Generate a new device for an existing user (provisioning), using
    /// pre-generated device seeds from the old device's bootstrap blob.
    ///
    /// The identity signing key is recovered from `identity_seed`;
    /// device keys use the provided seeds (must match the public keys
    /// that were sent to the server during approve).
    #[must_use]
    pub fn generate_new_device_from_seeds(
        user_id: Uuid,
        username: String,
        device_id: Uuid,
        identity_seed: [u8; 32],
        ...
        let device_signing_key = Ed25519Pair::from_secret_bytes(&device_signing_seed);
        let hpke_secret = x25519_dalek::StaticSecret::from(device_hpke_seed);
        let hpke_public = x25519_dalek::PublicKey::from(&hpke_secret).to_bytes();
        Self {
            user_id,
            username,
    ...
    ///
    /// Prefer [`generate_new_device_from_seeds`] when device keys have been
    /// pre-generated by the approving device (the bootstrap flow).
    #[must_use]
