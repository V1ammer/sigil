//! High-level messenger store backed by SQLCipher for Android.
...
use super::database::AndroidDatabase;
...
impl AndroidMessengerStore {
    ...
    pub fn new(db: AndroidDatabase) -> Self {
        Self { db }
    }

    /// Access the underlying database (for tests).
    pub fn db(&self) -> &AndroidDatabase {
        &self.db
    }
}
...
// ------------------------------------------------------------------
// Helpers (copied from desktop/messenger_store.rs)
// ------------------------------------------------------------------
